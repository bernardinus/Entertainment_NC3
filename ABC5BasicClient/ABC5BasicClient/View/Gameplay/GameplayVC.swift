...
class GameplayVC : UIViewController
{
    ...
    var players:[String] = ["A","B"]
    @IBOutlet var playersAvatar:[UIImageView]!
    ...
    override func viewDidLoad() {
        GameManager.shared.setupGame(sessionTime: 10, theme: .Fruits, numberOfPlayer: players.count)
        GameManager.shared.initializeSingleDeviceGame(players: players)
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        progressBar.progressClr = UIColor.red
    ...
    func startTimer()
    {
        progressBar.setProgressWithAnimation(duration: GameManager.shared.sessionTime, value: 1)        
    }
    ...
    func updateLayout()
    {
        ...
        for i in 0...7
        {
            let isAvailable = i < count
            playersAvatar[i].isHidden = !isAvailable
            playersName[i].isHidden = !isAvailable
            
            if( isAvailable )
            {
                playersName[i].text = players[i]
                playersName[i].textAlignment = .center
        ...
        recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest) { result, error in
            ...
            if let result = result
            {
                ...
                if !self.isChecked
                {
                    self.isChecked = true
                    print("Text \(answer)")
                    if(GameManager.shared.checkAnswer(answer: answer, timeleft: 10))
                    {
